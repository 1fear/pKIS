...
**Статус на 30.05.2026:** Backend API MVP выполнен на staging: реальные endpoints для health, active orders, scans, complete, imports, import history и day report работают; Postgres backup автоматизирован через systemd timer. Следующая граница - подключить desktop к backend через feature flag и вынести отдельные worker'ы.
